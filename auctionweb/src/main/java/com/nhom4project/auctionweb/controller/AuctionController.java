...
public class AuctionController {
    ...
    @PostMapping("/{id}/bid")
    public ResponseEntity<?> placeBid(@PathVariable Long id, @RequestParam Long bidderId, @RequestParam BigDecimal amount) {
        try {
            // In a real app, get bidder from session/security context
            Bidder bidder = new Bidder();
            bidder.setId(bidderId);
            
            boolean success = auctionService.placeBid(id, bidder, amount);
            return success ? ResponseEntity.ok("Bid placed successfully") : ResponseEntity.badRequest().body("Bid failed");
        } catch (Exception e) {
            return ResponseEntity.badRequest().body(e.getMessage());
        }
    }
